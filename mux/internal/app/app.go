...
import (
	...
	"github.com/miltonhit/mux/internal/agents"
	"github.com/miltonhit/mux/internal/app/messages"
	"github.com/miltonhit/mux/internal/tmux"
	agentsview "github.com/miltonhit/mux/internal/views/agents"
	"github.com/miltonhit/mux/internal/views/palette"
	"github.com/miltonhit/mux/internal/views/sessions"
	"github.com/miltonhit/mux/internal/views/windows"
	"github.com/miltonhit/mux/internal/views/worktrees"
	"github.com/miltonhit/mux/internal/worktree"
)

// Mode determines how mux starts.
type Mode int
...
func (m Model) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case messages.CreateWorktreeMsg:
		worktree.Create(msg.Branch)
		return m, tea.Quit

	case messages.RemoveWorktreeMsg:
		worktree.Remove(msg.Branch)
	...
	case messages.RunPopupMsg:
		tmux.DisplayPopup(msg.Command, msg.Width, msg.Height)
		return m, tea.Quit
	}
...
func (m Model) launchAgent(msg messages.LaunchAgentMsg) (tea.Model, tea.Cmd) {
	...
	for _, a := range agentsList {
		...
		for _, mode := range a.Modes {
			...
			switch msg.Target {
			case "split":
				tmux.SplitWindow(command)
			case "window":
				tmux.NewWindowWithCommand(a.Name, command)
			default: // "pane"
